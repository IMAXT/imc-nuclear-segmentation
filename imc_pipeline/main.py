...
def main(ref_channel=None, normalized_factor=None, imgFormat=None, imgFormatOut=None, catFormat=None, imgPath=None, output_key=None, output_key_ref=None, output_key_mask=None, output_key_cat=None, n_workers_param = None, memory_limit_param=None ):
    """IMC segmentation
    ...
    if not os.path.exists(outputPath_ref) : os.makedirs(outputPath_ref)
    if not os.path.exists(outputPath_mask): os.makedirs(outputPath_mask)
    if not os.path.exists(outputPath_cat) : os.makedirs(outputPath_cat)
    

    with LocalCluster(processes=True, n_workers = n_workers_param, memory_limit = memory_limit_param) as cluster: # use to minotir http://127.0.0.1:8787/status
    ...
    # with LocalCluster(processes=True, diagnostics_port=8787) as cluster:
        with Client(cluster) as client:
            ...
            for img_file in imgList:
                res = delayed(imcutil.process_image)(img_file, ref_channel, normalized_factor, outputPath_ref, outputPath_mask, outputPath_cat,imgFormat ,imgFormatOut, catFormat)
